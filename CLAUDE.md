...
Single-host Ansible project deploying Node-RED via Docker Compose on `nodered.local.iamrobertyoung.co.uk`. Secrets are stored in AWS SSM Parameter Store.
...
```bash
# Test connectivity
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible nodered -m ping

# Run full playbook
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml

# Run specific role only
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml --tags nodered
...
8. `nodered` - Node-RED deployment (Docker Compose with systemd service)

### Node-RED Role Structure
The `roles/nodered` role deploys Node-RED via Docker Compose:
- `tasks/main.yml` - Main task orchestration
- `defaults/main.yml` - Default variables
- `templates/docker-compose/` - Docker Compose configuration
- `templates/nginx/` - Nginx reverse proxy config (if applicable)
- `handlers/main.yml` - Service restart handlers

Key directories on target host:
- `/etc/nodered` - Configuration files and docker-compose.yml
- `/var/lib/nodered` - Application data
...
### Available Tags
Run specific parts with `--tags`: `configure-system`, `shell`, `docker`, `telegraf`, `step-ca-client`, `syslog`, `wazuh-agent`, `nodered`
